...
export async function promptUser(): Promise<ExtensionOptions> {
	p.intro(color.cyan('Welcome to zedx!'));
	...
	const group = await p.group(
		{
			...
			id: ({ results }) =>
				p.text({
					...
					placeholder: color.dim('my-zed-extension'),
					initialValue: results.name?.toLowerCase().replace(/\s+/g, '-') ?? 'my-zed-extension',
					validate: (value: string | undefined) => {
			...
			repository: ({ results }) =>
				p.text({
					...
					initialValue: color.dim(`https://github.com/${results.author ?? ''}/`)
				}),
			license: () =>
				p.select({
					message: 'License:',
