...
export async function addTheme(callerDir: string, themeName: string): Promise<void> {
    ...
    if (p.isCancel(appearance)) { p.cancel('Cancelled.'); process.exit(0) }
    ...
    const themeJson = await renderTemplate(
        ...
        { id: extensionId, author, themeName, appearances } as Record<string, unknown>
    )
...
export async function addLanguage(callerDir: string, languageId: string): Promise<void> {
    ...
    if (p.isCancel(languageName)) { p.cancel('Cancelled.'); process.exit(0) }
